...
| 5 · Campaign Targeting Rework | Richer goal capture, structured (non-semantic) filtering on LinkedIn-derived title/seniority/function/industry, wider candidate pool, in-app candidate detail + LinkedIn link | Phase 1 (extends the LinkedIn importer) | ✅ Done (M28–M34) |
...
- **CSV-row-to-person matching** (for updating existing people vs. creating new ones): an exact email match against an existing Gmail/Hotmail contact attaches the new LinkedIn contact to that person directly — unambiguous, so no review needed. Everything else is name-based and **never auto-merged**: the existing merge-suggestion engine (already source-agnostic) now also accounts for common nickname equivalence ("Rob"/"Robert", "Nick"/"Nicholas") alongside its existing exact/fuzzy/single-initial matching, but a nickname-aided match is surfaced as a suggestion on `/merges` for manual review exactly like every other merge in the app — the app has no auto-merge path anywhere, and this doesn't add one. Once a person has been matched once, later re-imports match directly on our own previously-stored LinkedIn URL first (fast path), falling back to the email/name matching above only for people not yet seen.

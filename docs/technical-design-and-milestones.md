...
| M31 | CSV-row-to-person matching + nickname-aware merge suggestions | ✅ Done | 5 | M28 | A CSV row with a matching email attaches to the existing Person; a name-only match never auto-merges, only appears on `/merges` |
| M32 | Deterministic structured filter | ✅ Done | 5 | M28–M31 | Filtering with no criteria returns every Person with an active LinkedIn contact regardless of message history |
| M33 | LLM goal→filter drafting | ✅ Done | 5 | M32 | A goal like "hiring enterprise PMs" drafts an editable filter with `function: product_management` pre-checked |
| M34 | Candidate table polish: sort, warnings, drawer | ✅ Done | 5 | M32 | Sorting/threshold warnings work via query params; the drawer's "Open LinkedIn" opens the right profile in a new tab |
...
**Part A (primary path)**: `attachContactToExistingPerson(db, source, identity, personId, status)` in `src/lib/contact-resolution.ts` — inserts a `contact` row against a *given* `personId` instead of always creating a new solo person. Alongside it, `findContactBySourceIdentifier(db, source, identifier)` — a read-only peek exposing the "does this Contact already exist" check `findOrCreateContact` does internally, so a caller can branch on it before deciding whether to try the email-match path.

**Shipped as** a change to `importLinkedInConnections`'s per-row loop in `linkedin-import.ts`: peek for an existing `(linkedin, profileUrl)` Contact first (the ordinary re-import fast path); if not found and `row.email` is present, look it up via `findContactByEmail` (already existed, from M24) — a match attaches via `attachContactToExistingPerson`; no match (or no email) falls through to today's `findOrCreateContact` (creates a new Person), unchanged.

**Part B (fallback path, never auto-merges)**: `src/lib/nickname-equivalence.ts` — a static table of ~35 nickname groups (Rob/Robert/Bob, Nick/Nicholas, Bill/William/Liam, etc. — a starter list, explicitly non-exhaustive, append-as-you-go), with `namesAreNicknameEquivalent(a, b)`. One deviation from the initial sketch worth noting: a name that's colloquially short for *two* different names (e.g. "Ted" for both Edward and Theodore) can only live in one group in this table's simple `name -> group index` structure — resolved by keeping "Ted"/"Teddy" under Theodore only and dropping them from Edward's group, rather than building a more complex multi-group structure for a rare ambiguity.

**Shipped as** `namesMatchViaNickname(nameA, nameB)` in `merge-suggestions.ts` — same 2-token shape as `namesMatchViaSingleInitial`, but requires an *exact* last-name match and a nickname-group first-name match. Wired into `scoreContactPair` as a new branch (checked after `exact_name_match`, before `namesMatchViaSingleInitial`), scored `0.53` (`name_nickname_match` reason) — just above single-initial's `0.52`, since a known nickname substitution is more specific evidence than a bare initial. No changes needed to `findCandidatePairs`'s bucketing — a nickname pair always shares the (exact) last-name token, so it already lands in the same candidate bucket. Confirmed during design and unchanged here: the app has no auto-merge path anywhere (every merge on `/merges` is user-confirmed) — nickname-aided matches, like every other name-based signal, only ever surface as a suggestion.

**Doc fix**: `docs/outreach-roadmap.md`'s Phase 5 section previously said CSV matching would "auto-merge only when exactly one confident candidate is found" — stale from before this was resolved. Corrected to describe the actual behavior (email exact-match auto-attaches; everything else is suggestion-only, nickname-aided or not).

**Test plan** (as built): integration (pglite, `contact-resolution.test.ts`) — `findContactBySourceIdentifier` finds/doesn't-find correctly, including not matching the same identifier under a different source; `attachContactToExistingPerson` creates a Contact under the given Person, not a new one. Integration (pglite, `linkedin-import.test.ts`) — a row with an email matching an existing Gmail contact's Person attaches there without creating a new Person; a row with no matching email still creates a new Person (regression). Unit (`nickname-equivalence.test.ts`) — known pairs match case-insensitively, unrelated names don't, the Ted/Edward-vs-Ted/Theodore disambiguation holds. Unit (`merge-suggestions.test.ts`) — "Rob Smith"/"Robert Smith" scores via `name_nickname_match`; a same-last-name-different-first-name pair that isn't a known nickname pair and isn't close enough for the separate typo-tolerance path doesn't fire.

### M32 — Deterministic structured filter ✅

Core of Phase 5 — everything before it is groundwork.

**Shipped as** `src/lib/candidate-filter.ts`: `StructuredFilter` (`titleQuery`, `seniority[]`, `function[]`, `industry[]` — OR within a field, AND across fields) and `filterCandidates(db, filter)`. Query: any `person` inner-joined to an *active* `linkedin` `contact` — no event-join requirement (the actual behavior change from the old embedding-ranking-based `loadCandidates` in `campaign-ranking.ts`, deleted this milestone). `titleQuery` is a case-insensitive `ilike` against `standardizedTitle`; the three enum fields use `inArray`. Last-interaction date isn't computed here — `listPeopleByLastTouched` (M26) is called once and merged in via a `Map<personId, lastTouchedAt>` lookup, reusing that module's aggregate query rather than re-deriving it. Default sort `person.name`. Sorting controls, broad/empty-result warnings, and the candidate drawer are explicitly **M34's** job, not this one's — M32 is the filter plus a plain results table.

**Wiring**: `src/app/campaigns/page.tsx` swaps the old single goal `<input>` (GET, embedding-ranked) for a structured filter form — text input for `titleQuery`, checkboxes for `seniority`/`function`/`industry` (kept as checkboxes rather than a native multi-select — the app's existing precedent for multi-value selection, and more discoverable without JS). Results render as a `<table>` (name/title/company/seniority/function/industry/last-interaction columns) instead of the old scored `<ol>`. `goal` moves out of the GET query params entirely — it's no longer used for targeting, only for drafting, so it becomes a plain text input *inside* the POST campaign-creation form (shown only on the "new campaign" path — adding to an *existing* campaign already drafts against `campaignRow.goal`, the campaign's own persisted value, never a page param). The "Add more people" link on `/campaigns/[id]` was updated to drop the now-unused `goal` query param it used to pass.

**Scope cut, not in the original plan**: the per-result "Draft one-off outreach" link (previously prefilled with the page's current `goal` query param) was removed rather than kept without a goal value — `/campaigns/draft` requires a goal and errors without one, and there's no natural goal value available at filter time anymore (goal now only exists inside the creation form, submitted after results are already shown). Revisit if this turns out to matter in practice; it wasn't the focus of this milestone.

**Removed**: `src/lib/campaign-ranking.ts` and its test file, `scripts/rank-campaign.ts` (and its `db:rank-campaign` npm script) — confirmed dead once `page.tsx` stopped calling `rankPeopleForCampaign`. `src/lib/campaigns.ts`'s `addRecipients` no longer imports `CampaignRankingEntry` for its `people` param type — changed to a local `{ personId: number }[]`, decoupling it from the (now-deleted) ranking module entirely rather than leaving a type-only import behind. The underlying embedding infrastructure (`embeddings.ts`, `person-embedding.ts`, `person.summaryEmbedding`) is untouched — only its one campaign-targeting consumer was removed.

**Test plan** (as built): integration (pglite, `candidate-filter.test.ts`) — empty filter returns everyone with an active LinkedIn contact, **including a person with zero events** (the key regression test for the event-requirement removal); a `pending` LinkedIn contact is excluded; a person with no LinkedIn contact at all is excluded; each filter field narrows correctly in isolation and ANDs together across fields; multiple values within one field OR together; default sort is by name; `lastInteractionAt` is populated from real event history and `null` when there's none; empty-result case returns `[]`.

### M33 — LLM goal→filter drafting + review/edit step ✅

**Shipped as** `src/lib/filter-drafting.ts`: `buildFilterDraftPrompt(goal)` / `parseFilterDraftResponse(raw)` (pure, tool-use, enum-constrained) and `draftFilterFromGoal(goal)` (full-pipeline). New `draftFilterAction` (same FormData → lib call → redirect-with-URLSearchParams pattern as `createCampaignAction`) pre-fills M32's checkbox form with the drafted filter's fields — "review and edit" is the checkboxes arriving pre-checked, no new UI surface. `goal` comes back as a page-level query param for this milestone (M32 had moved it out entirely) — not a contradiction, just a second use of the same value: this form drafts a filter from it, and the same value round-trips through the redirect to also pre-fill the bottom campaign-creation form's goal input, so it isn't retyped.

**Parsing differs from title/industry classification on purpose**: `parseFilterDraftResponse` drops an out-of-taxonomy value from an array instead of substituting a fallback like `unknown`/`other` — a filter has no "unknown" concept, and a dimension the model isn't confident about should come back empty (no filter on it) rather than populated with a value that would only match people already tagged unknown.

**Taxonomy-prompt refactor, done as part of this milestone rather than deferred**: `title-extraction.ts` and `industry-inference.ts` previously each inlined their own copy of the seniority/function and industry guidance tables. `filter-drafting.ts` needs all three in one prompt, so copying them a third time was exactly the drift risk flagged when M28/M29 shipped. New `src/lib/taxonomy-prompts.ts` exports `SENIORITY_FUNCTION_GUIDANCE`/`INDUSTRY_GUIDANCE` as plain text (moved verbatim, not reworded); `title-extraction.ts`/`industry-inference.ts` now compose their `SYSTEM_PROMPT` from the shared constant plus their own task-specific framing/examples/ambiguity-rule text — confirmed via their existing test suites (21 tests, unchanged) that the refactor didn't alter prompt content.

**Test plan** (as built): unit (`filter-drafting.test.ts`) — prompt builder includes both guidance blocks and the "omitted dimension is valid" instruction; parser extracts a full multi-dimension filter, omits a dimension entirely when the model returns it empty (not an empty array left in the object), drops an out-of-taxonomy value from an array without substituting a fallback, omits a dimension when every value in it is invalid, trims/omits `titleQuery`, throws with no tool call. Manual — `npm run db:draft-filter -- "goal text"` against real goal strings (inherently judgment-based, no automated test for the impure LLM call itself, per this doc's own conventions).

### M34 — Candidate table polish: sortable columns, warnings, side drawer ✅

**Shipped as** a new pure `sortFilterResults(results, sort, dir)` in `candidate-filter.ts`, called from `src/app/campaigns/page.tsx` only when a `?sort=name|title|company|seniority|function|industry|lastInteraction&dir=asc|desc` param is present — `filterCandidates` itself keeps its existing `ORDER BY person.name` as the default, unchanged. Sorting couldn't be pushed into that SQL query for every field: `lastInteractionAt` only exists after `filterCandidates`'s JS merge step (M26's `listPeopleByLastTouched`), so it can never be a SQL-orderable column — rather than split sort logic across two layers, all seven fields sort uniformly in this one function. Nulls sort last in both directions (same principle as `listPeopleByLastTouched` itself). Column headers are plain `<a>` links via a `buildSortUrl(field)` helper in `page.tsx` (same pattern as M26's `/people` sort links) that preserves every other current param and toggles `dir` on a second click of the same column. New `BROAD_RESULT_THRESHOLD` (200, placeholder) lives in `candidate-filter.ts`; `page.tsx` shows a warning above the table past that count, alongside the existing (M32) 0-result message.

New `src/app/campaigns/CandidateDrawerTrigger.tsx`, `"use client"` — this app's **4th** Client Component (after `CopyButton.tsx`, `ImportSubmitButton.tsx`, `LinkedInConnectionsImportForm.tsx`), same narrow-leaf pattern: one instance per table row, each owning its own `useState` open/closed, rather than one client component wrapping the whole table — the table's checkboxes needed to stay part of the server-rendered POST form exactly as M32 built it. Renders a "Details" button opening a fixed-position side panel with every field plus `<a href={linkedinProfileUrl} target="_blank" rel="noopener noreferrer">Open LinkedIn</a>` (new tab — LinkedIn blocks iframing, a hard constraint, not a preference). `FilterResult` gained a non-nullable `linkedinProfileUrl` field (the join key from the existing active-LinkedIn-contact `INNER JOIN`, just not previously selected) to supply it.

**Test plan** (as built): unit (`candidate-filter.test.ts`) — `sortFilterResults` sorts ascending/descending correctly per field, nulls sort last under both directions, an all-null field doesn't throw, the input array isn't mutated; `linkedinProfileUrl` is populated correctly on an existing `filterCandidates` test. Manual only for the drawer itself (no component-test framework exists in this repo) and for the sort-link/threshold-warning UI end to end.
